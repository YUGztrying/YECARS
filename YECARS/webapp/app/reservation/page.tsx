...
export default function ReservationPage() {
    ...
    const handleSubmitReservation = async () => {
        ...
        try {
            ...
            router.push(`/confirmation?reservation_id=${data.id}`);
                            ...
                            {STEPS.map((step, index) => (
                                <React.Fragment key={step.id}>
                                    <div className="flex flex-col items-center">
                                        <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${currentStep >= step.id
                                            ? 'bg-blue-600 border-blue-600 text-white shadow-lg'
                                            : 'bg-white border-slate-300 text-slate-400'
                                            }`}>
                                            <step.icon className="w-4 h-4 md:w-6 md:h-6" />
                                        </div>
                                        <span className={`mt-2 text-xs md:text-sm font-medium ${currentStep >= step.id ? 'text-blue-600' : 'text-slate-400'
                                            }`}>
                                            {step.title}
                                        </span>
                                    </div>
                                    {index < STEPS.length - 1 && (
                                        <div className={`h-0.5 w-12 md:w-24 transition-all duration-300 ${currentStep > step.id ? 'bg-blue-600' : 'bg-slate-200'
                                            }`} />
                                    )}
                                </React.Fragment>
                            ))}
